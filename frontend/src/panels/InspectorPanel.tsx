...
import { AlertTriangle, Crosshair, Play, X } from 'lucide-react';

import { api } from '../api/client';
import { getLayer } from '../layers/registry';
import { formatAltitude, formatCoord, formatTime } from '../lib/format';
...
export function InspectorPanel() {
  ...
  const layer = getLayer(selected.layer);
          ...
          <div className="flex flex-wrap gap-1">
            {modules.map((m) => (
              <Button
                ...
                size="sm"
                variant="ghost"
                onPress={() => void api.runModule(m.id, { entity_type: entityType, value: selected.name }).catch((e: Error) => console.warn(e.message))}
              >
                <Play size={12} /> {m.name}
              </Button>
            ))}
